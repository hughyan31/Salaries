...
def plots(df):
    #Plot 1: distribution plots for salary
    Salary = df["salary_in_usd"]
    name = df["salary_in_usd"].name
    Rfig = make_subplots(rows=1, cols=2, subplot_titles=('Percentages', 'Countplot'))
    fig1 = ff.create_distplot([Salary], [name], bin_size=5000, colors=['#A56CC1'], show_hist=False,
    ...
    #Plot 2: Pie charts for multiple categorical data
    ...
    Rfig2 = make_subplots(rows=2, cols=2, subplot_titles=("Experience Level", "Company Size", "Work Year", "Remote Ratio"),
    ...
    #Plot 3: Bar Chart for popular job titles
    ...
    #Plot 4: Bar Chart for mean salary by work year
    ...
    Rfig4.update_layout(showlegend=False)
    Rfig4.write_html("SalaryByWorkyear.html")

    #Bar Chart for mean salary by experience level
...
def salary_range_models(data):
    #split the salary by quantiles for better classification
    ...
    #spliting the data to train set andtest set
    X_train, X_test, y_train, y_test = train_test_split(data, y, test_size=0.2)
    
    #Comparing the performance of Logistic Regression,Random Forest and Histogram Based Gradient Boosting
    classifiers = [
    ...
    for name, clf in classifiers:
        ...
        if accuracy > best_score:
            ...
            best_model = pipeline
    """
    #Use gridsearch to find best hyperparameters for the best model
    ...
    #Plot the confusion matrix and classification report for the best model
    titles_options = [
...
#Change some of the categorical data for better understanding
...
#Change the countries label from alpha_2(ISO 3166) to actual name 
country_map = {}
...
#Using OrdinalEncoder to encode categorical features as an integer array
categorical_features = ['experience_level', 'employment_type', 'employee_residence', 'company_location',
                        'company_size', 'remote_ratio']
encoder = OrdinalEncoder()
filtered_data[categorical_features] = encoder.fit_transform(filtered_data[categorical_features])
